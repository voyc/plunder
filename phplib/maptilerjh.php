...
/*
	...
	Changes made below marked with //jh
	The original code supports only zoom sizes as even powers of 2.
	And I want only zoom 2^5.16998 approximately.
	Our image is 10800 by 5400 and tilesize is 300 by 300.
	Resulting in 18 rows and 36 columns.
*/

//init
$map_tiler = new MapTiler('/home/jhagstrand/natural_earth_data/raster/NE2_50M_SR_W.tif', array(
  'tiles_path' => '../html/assets/tiles/',
  'tile_size' => 300,
  'format' => 'png',
  'zoom_min' => 0,
  'zoom_min' => 0,
  'tms' -> false    // if true, y coordinate goes from south to north
));
//execute
try {
...
class MapTiler
{
	 ...
	 */
	//jh protected $store_structure = '%d/%d/%d';
	protected $store_structure = 'ne2_c%d_r%d';
	...
	public function tilesForZoom($zoom) {
		...
		for($ix = 0; $ix < $x; $ix++){
			...
			for($iy = 0; $iy < $y; $iy++){
				//full file path
				//jh $lvl_file = $this->tiles_path.'/'.sprintf($this->store_structure, $zoom, $ix, $iy).'.'.$ext;
				$lvl_file = $this->tiles_path.'/'.sprintf($this->store_structure, $ix, $iy).'.'.$ext;
